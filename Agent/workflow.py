...
from Agent.state import AgentState, RouteDecision, EvidenceGrade
...
def _extract_text(content) -> str:
    """response.content from langchain-google-genai can be a plain string
    or a list of content blocks (e.g. Gemini 3.x's thought-signature
    format). Normalize either shape into plain text."""
...
def rewrite_query(state: AgentState):
    rewritten = _extract_text(llm().invoke(f"""
...
""")
    ...
    print("metadata:", response.response_metadata, flush=True)
    print("content repr:", repr(response.content), flush=True)
    print("===========================", flush=True)
...
def generate_from_web(state: AgentState):
    answer = _extract_text(llm().invoke(f"""
